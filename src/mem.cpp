...
void* find_virtual_address(void* pa, LIST_ENTRY* mappings) {
  ...
  while (le != mappings) {
    ...
    if (m->va) {
      if ((uint8_t*)pa >= (uint8_t*)m->pa &&
          (uint8_t*)pa < (uint8_t*)m->pa + (m->pages * EFI_PAGE_SIZE))
...
static EFI_STATUS map_memory(EFI_BOOT_SERVICES* bs,
                             LIST_ENTRY*        mappings,
                             uintptr_t          va,
                             uintptr_t          pa,
                             unsigned int       pages) {
  ...
  if (pae) {
    do {
      HARDWARE_PTE_PAE* dir =
          (HARDWARE_PTE_PAE*)(pdpt[va >> 30].PageFrameNumber * EFI_PAGE_SIZE);
      ...
      if (!dir[index].Valid) {  // allocate new page table
        ...
        Status =
            bs->AllocatePages(AllocateAnyPages, EfiBootServicesData, 1, &addr);
        if (EFI_ERROR(Status)) {
        ...
        page_table =
            (HARDWARE_PTE_PAE*)(dir[index].PageFrameNumber * EFI_PAGE_SIZE);
    ...
    do {
      ...
      if (!page_directory[index].Valid) {  // allocate new page table
        ...
        page_table = (HARDWARE_PTE*)(page_directory[index].PageFrameNumber *
                                     EFI_PAGE_SIZE);
  ...
  do {
    ...
    if (!pml4[index].Valid) {
      ...
      Status = add_mapping(bs, mappings, NULL, (void*)(uintptr_t)addr, 1,
                           LoaderMemoryData);
      if (EFI_ERROR(Status)) {
      ...
      /* Somewhat surprising behaviour from gcc here - combining the following
       * two lines into one results in the value being sign-extended. Compiler
       * bug? */
...
EFI_STATUS add_mapping(EFI_BOOT_SERVICES* bs,
                       LIST_ENTRY*        mappings,
                       void*              va,
                       void*              pa,
                       unsigned int       pages,
  ...
  while (le != mappings) {
    ...
    if ((m2->pa >= m->pa && pa2_end <= pa_end) ||
        m2->pages == 0) {  // remove block entirely
...
EFI_STATUS process_memory_map(EFI_BOOT_SERVICES* bs,
                              void**             va,
                              LIST_ENTRY*        mappings) {
  EFI_STATUS             Status;
  UINTN                  key, count;
  ...
  do {
    ...
    Status =
        bs->GetMemoryMap(&efi_map_size, desc, &key, &map_desc_size, &version);
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (desc->PhysicalStart + (desc->NumberOfPages << EFI_PAGE_SHIFT) >
        0xffffffff)
      desc->NumberOfPages =
          (0x100000000 - desc->PhysicalStart) >> EFI_PAGE_SHIFT;
#endif
    ...
    if (memory_type != LoaderFree) {
      Status =
          add_mapping(bs, mappings, va2, (void*)(uintptr_t)desc->PhysicalStart,
      ...
      if (desc->PhysicalStart <= 0xa0000 &&
          desc->PhysicalStart + (desc->NumberOfPages << EFI_PAGE_SHIFT) >
              0xa0000)
        map_video_ram = false;
      ...
      Status =
          add_mapping(bs, mappings, NULL, (void*)(uintptr_t)desc->PhysicalStart,
    ...
    if (desc->Type == EfiLoaderCode) {
      Status = add_mapping(bs, mappings, (void*)(uintptr_t)desc->PhysicalStart,
                           (void*)(uintptr_t)desc->PhysicalStart,
  ...
  if (map_video_ram) {
    Status = add_mapping(bs, mappings, NULL, (void*)0xa0000, 0x60,
  ...
  if (map_first_page) {
    Status =
        add_mapping(bs, mappings, NULL, (void*)0, 1, LoaderFirmwarePermanent);
    if (EFI_ERROR(Status)) {
      print_error("add_mapping", Status);
      return Status;
    }
...
static EFI_STATUS setup_memory_descriptor_list(LIST_ENTRY* mappings,
                                               LIST_ENTRY& mdl,
                                               void*       pa,
                                               void*       va) {
  LIST_ENTRY* le;
  ...
  while (le != &mdl) {
    MEMORY_ALLOCATION_DESCRIPTOR* mad =
        _CR(le, MEMORY_ALLOCATION_DESCRIPTOR, ListEntry);
    MEMORY_ALLOCATION_DESCRIPTOR* mad2 =
        _CR(le->Flink, MEMORY_ALLOCATION_DESCRIPTOR, ListEntry);

    if (le->Flink == &mdl)
      break;

    if (mad->BasePage + mad->PageCount == mad2->BasePage &&
        mad->MemoryType == mad2->MemoryType) {
...
static EFI_STATUS allocate_mdl(EFI_BOOT_SERVICES* bs,
                               LIST_ENTRY*        mappings,
                               void*              va,
                               void**             pa,
                               size_t*            mdl_pages) {
  ...
  Status =
      bs->AllocatePages(AllocateAnyPages, EfiBootServicesData, pages, &addr);
  if (EFI_ERROR(Status)) {
    print_error("AllocatePages", Status);
    return Status;
  }
...
#ifdef _X86_
// Vista seems to assume that all our virtual addresses are the physical
// addresses plus 0x80000000. Here we trawl through the memory map to find an
// address which satisfies this.
static EFI_STATUS find_cr3(EFI_BOOT_SERVICES*    bs,
                           void*                 va,
                           EFI_PHYSICAL_ADDRESS* addr) {
  EFI_STATUS             Status;
  ...
  for (unsigned int i = 0; i < count; i++) {
    if (desc2->Type == EfiConventionalMemory) {
      if (desc2->PhysicalStart + ((desc2->NumberOfPages - 1) * EFI_PAGE_SIZE) >=
          pa) {
        ...
        Status =
            bs->AllocatePages(AllocateAddress, EfiBootServicesData, 1, addr);
        if (EFI_ERROR(Status)) {
          print_error("AllocatePages", Status);
          return Status;
        }
...
#ifdef __x86_64__
// HAL on amd64 wants page directories set up for the last four megabytes of VA
// space
static EFI_STATUS add_hal_mappings(EFI_BOOT_SERVICES* bs,
                                   LIST_ENTRY*        mappings) {
  HARDWARE_PTE_PAE* pdpt;
  HARDWARE_PTE_PAE* pd;
  ...
  for (unsigned int i = 0; i < 2; i++) {
    if (!pd[((HAL_MEMORY >> 21) & 0x1ff) + i].Valid) {
      ...
      pd[((HAL_MEMORY >> 21) & 0x1ff) + i].PageFrameNumber =
          addr / EFI_PAGE_SIZE;
...
EFI_STATUS map_efi_runtime(EFI_BOOT_SERVICES* bs,
                           LIST_ENTRY*        mappings,
                           void*&             va,
                           uint16_t           version) {
  EFI_STATUS             Status;
   ...
   * SetVirtualAddressMap. See https://lwn.net/Articles/444666/ for Matthew
   * Garrett's dispassionate discussion of the issue. */
  ...
  Status = bs->AllocatePages(AllocateAnyPages, EfiBootServicesData,
                             page_count(efi_runtime_map_size), &addr);
  if (EFI_ERROR(Status)) {
    print_error("AllocatePages", Status);
    return Status;
  }
  ...
  for (unsigned int i = 0; i < efi_map_size / map_desc_size; i++) {
    if (desc->Attribute & EFI_MEMORY_RUNTIME ||
      ...
      if (desc->Attribute & EFI_MEMORY_RUNTIME) {
        ...
        Status =
            add_mapping(bs, mappings, va, (void*)(uintptr_t)desc->PhysicalStart,
  ...
  if (version >= _WIN32_WINNT_WINBLUE) {
    Status =
        add_mapping(bs, mappings, va, (void*)(uintptr_t)efi_runtime_map,
...
EFI_STATUS enable_paging(EFI_HANDLE         image_handle,
  ...
  Status =
      add_mapping(bs, mappings, stack, stack, STACK_SIZE, LoaderOsloaderStack);
  if (EFI_ERROR(Status)) {
    print_error("add_mapping", Status);
    return Status;
  }
  ...
  if (pae) {
    ...
    Status =
        bs->AllocatePages(AllocateMaxAddress, EfiBootServicesData, 4, &addr);
    if (EFI_ERROR(Status)) {
      print_error("AllocatePages", Status);
      return Status;
    }
  ...
  Status = bs->AllocatePool(EfiLoaderData,
                            num_entries * sizeof(EFI_MEMORY_DESCRIPTOR),
                            (void**)&mapdesc);
  if (EFI_ERROR(Status)) {
    print_error("AllocatePool", Status);
    return Status;
  ...
  if (pae) {
    HARDWARE_PTE_PAE* dir =
        (HARDWARE_PTE_PAE*)(pdpt[SELFMAP >> 30].PageFrameNumber *
                            EFI_PAGE_SIZE);

    for (unsigned int i = 0; i < 4; i++) {
      dir[(SELFMAP >> 21 & 0x1ff) + i].PageFrameNumber =
          pdpt[i].PageFrameNumber;
      dir[(SELFMAP >> 21 & 0x1ff) + i].Valid = 1;
      dir[(SELFMAP >> 21 & 0x1ff) + i].Write = 1;
    }
  } else {
    page_directory[SELFMAP >> 22].PageFrameNumber =
        (uintptr_t)page_directory / EFI_PAGE_SIZE;
    page_directory[SELFMAP >> 22].Valid = 1;
    page_directory[SELFMAP >> 22].Write = 1;
  }
#elif defined(__x86_64__)
  pml4[(SELFMAP & 0xff8000000000) >> 39].PageFrameNumber =
      (uintptr_t)pml4 / EFI_PAGE_SIZE;
  ...
  // do mappings
  {
    ...
    while (le != mappings) {
      ...
      if (m->va) {
        if ((uint8_t*)systable >= (uint8_t*)m->pa &&
            (uint8_t*)systable < (uint8_t*)m->pa + (m->pages * EFI_PAGE_SIZE))
          new_ST = (EFI_SYSTEM_TABLE*)((uint8_t*)systable - (uint8_t*)m->pa +
                                       (uint8_t*)m->va);

        Status = map_memory(bs, mappings, (uintptr_t)m->va, (uintptr_t)m->pa,
                            m->pages);
        if (EFI_ERROR(Status)) {
  ...
  if (ft_pool) {
    Status = map_memory(bs, mappings, (uintptr_t)ft_pool, (uintptr_t)ft_pool,
                        FT_POOL_PAGES);
    if (EFI_ERROR(Status)) {
      print_error("map_memory", Status);
      return Status;
  ...
  if (shadow_fb) {
    Status = map_memory(bs, mappings, (uintptr_t)shadow_fb,
                        (uintptr_t)shadow_fb, page_count(framebuffer_size));
    if (EFI_ERROR(Status)) {
      print_error("map_memory", Status);
      return Status;
    }
  ...
  if (efi_runtime_map) {
    ...
    while ((uint8_t*)desc < (uint8_t*)efi_runtime_map + efi_runtime_map_size) {
      Status = map_memory(bs, mappings, (uintptr_t)desc->PhysicalStart,
                          (uintptr_t)desc->PhysicalStart, desc->NumberOfPages);
      if (EFI_ERROR(Status)) {
        print_error("map_memory", Status);
        return Status;
      }
    ...
    Status = map_memory(bs, mappings, (uintptr_t)efi_runtime_map,
                        (uintptr_t)efi_runtime_map,
...
#ifdef _X86_
  if (pae) {  // map cr3
    Status = map_memory(bs, mappings, ((uintptr_t)pdpt + MM_KSEG0_BASE),
                        (uintptr_t)pdpt, 1);
    if (EFI_ERROR(Status)) {
      print_error("map_memory", Status);
      return Status;
  ...
  if (pae) {
    for (unsigned int i = 0; i < 4; i++) {
      HARDWARE_PTE_PAE* dir =
          (HARDWARE_PTE_PAE*)(pdpt[i].PageFrameNumber * EFI_PAGE_SIZE);

      for (unsigned int j = 0; j < EFI_PAGE_SIZE / sizeof(HARDWARE_PTE_PAE);
           j++) {
        if (dir[j].Valid) {
          Status = add_mapping(
              bs, mappings, NULL,
              (void*)(uintptr_t)(dir[j].PageFrameNumber * EFI_PAGE_SIZE), 1,
              LoaderMemoryData);
          if (EFI_ERROR(Status)) {
            print_error("add_mapping", Status);
            return Status;
    ...
    for (unsigned int i = 0; i < EFI_PAGE_SIZE / sizeof(HARDWARE_PTE); i++) {
      if (page_directory[i].Valid) {
        Status =
            add_mapping(bs, mappings, NULL,
                        (void*)(uintptr_t)(page_directory[i].PageFrameNumber *
                                           EFI_PAGE_SIZE),
                        1, LoaderMemoryData);
        if (EFI_ERROR(Status)) {
          print_error("add_mapping", Status);
  ...
  Status =
      map_memory(bs, mappings, (uintptr_t)va, (uintptr_t)mdl_pa, mdl_pages);
  if (EFI_ERROR(Status)) {
    print_error("map_memory", Status);
    return Status;
  }
...
void merge_mappings(LIST_ENTRY* mappings) {
  ...
  while (le != mappings) {
    ...
    if (m->type == LoaderFree && m2->type == LoaderFree && !m->va && !m2->va &&
        m2->pa == (uint8_t*)m->pa + (m->pages * EFI_PAGE_SIZE)) {
      m->pages += m2->pages;
      RemoveEntryList(&m2->list_entry);
      continue;
    }
