...
static EFI_STATUS EFIAPI Load(EFI_FILE_HANDLE File,
                              void*           VirtualAddress,
                              EFI_PE_IMAGE**  Image);
...
EFI_STATUS pe_register(EFI_BOOT_SERVICES* BootServices, uint32_t seed) {
  ...
  return bs->InstallProtocolInterface(&pe_handle, &pe_guid,
...
static EFI_STATUS EFIAPI get_entry_point(EFI_PE_IMAGE* This,
                                         void**        EntryPoint) {
  pe_image*         img = _CR(This, pe_image, pub);
  ...
  nt_header =
      (IMAGE_NT_HEADERS*)((uint8_t*)img->pub.Data + dos_header->e_lfanew);

  if (nt_header->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
    *EntryPoint =
        (uint8_t*)img->va + nt_header->OptionalHeader64.AddressOfEntryPoint;
  else
    *EntryPoint =
        (uint8_t*)img->va + nt_header->OptionalHeader32.AddressOfEntryPoint;

  return EFI_SUCCESS;
}

static EFI_STATUS EFIAPI list_imports(EFI_PE_IMAGE*    This,
                                      EFI_IMPORT_LIST* ImportList,
                                      UINTN*           BufferSize) {
  pe_image*                img = _CR(This, pe_image, pub);
  IMAGE_DOS_HEADER*        dos_header;
  IMAGE_NT_HEADERS*        nt_header;
  ...
  if (nt_header->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
    if (nt_header->OptionalHeader64.NumberOfRvaAndSizes <=
            IMAGE_DIRECTORY_ENTRY_IMPORT ||
        nt_header->OptionalHeader64.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]
                .VirtualAddress == 0 ||
        nt_header->OptionalHeader64.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]
    ...
    iid = (IMAGE_IMPORT_DESCRIPTOR*)((uint8_t*)img->pub.Data +
    ...
    if (nt_header->OptionalHeader32.NumberOfRvaAndSizes <=
            IMAGE_DIRECTORY_ENTRY_IMPORT ||
        nt_header->OptionalHeader32.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]
                .VirtualAddress == 0 ||
        nt_header->OptionalHeader32.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]
                .Size < sizeof(IMAGE_IMPORT_DESCRIPTOR)) {
      *BufferSize = 0;
  ...
  for (unsigned int i = 0; i < total_entries; i++) {
    ...
    for (unsigned int j = 0; j < i; j++) {
      if (!stricmp((char*)((uint8_t*)img->pub.Data + iid[i].Name),
                   (char*)((uint8_t*)img->pub.Data + iid[j].Name))) {
        dupe = true;
        break;
      }
    }
  ...
  *BufferSize = needed_size;
  ...
  next_text =
      offsetof(EFI_IMPORT_LIST, Imports[0]) + (num_entries * sizeof(UINT32));
  pos = 0;
...
static EFI_STATUS resolve_imports2_64(
    pe_image*                    img,
    pe_image*                    img2,
    IMAGE_EXPORT_DIRECTORY*      export_dir,
    uint64_t*                    orig_thunk_table,
    uint64_t*                    thunk_table,
    EFI_PE_IMAGE_RESOLVE_FORWARD ResolveForward) {
  EFI_STATUS        Status;
  IMAGE_DOS_HEADER* dos_header2 = (IMAGE_DOS_HEADER*)img2->pub.Data;
  IMAGE_NT_HEADERS* nt_header2 =
      (IMAGE_NT_HEADERS*)((uint8_t*)img2->pub.Data + dos_header2->e_lfanew);
  uint16_t* ordinal_table =
      (uint16_t*)((uint8_t*)img2->pub.Data + export_dir->AddressOfNameOrdinals);
  uint32_t* name_table =
      (uint32_t*)((uint8_t*)img2->pub.Data + export_dir->AddressOfNames);
  uint32_t* function_table =
      (uint32_t*)((uint8_t*)img2->pub.Data + export_dir->AddressOfFunctions);
  ...
  while (*orig_thunk_table) {
    char* name =
        (char*)((uint8_t*)img->pub.Data + *orig_thunk_table + sizeof(uint16_t));
    uint32_t index;
    uint16_t ordinal;
    void*    func;
    bool     found = false;
    ...
    if (*orig_thunk_table & 0x8000000000000000)
      ordinal = (*orig_thunk_table & ~0x8000000000000000) -
                1;  // FIXME - make sure not out of bounds
    else {
    ...
    if ((nt_header2->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC &&
         function_table[ordinal] >=
             nt_header2->OptionalHeader64
         ...
         function_table[ordinal] <
             nt_header2->OptionalHeader32
                 ...
                 nt_header2->OptionalHeader32
                     ...
                     .Size)) {  // forwarded
      char* redir_name =
          (char*)((uint8_t*)img2->pub.Data + function_table[ordinal]);
...
static EFI_STATUS resolve_imports2_32(
    pe_image*                    img,
    pe_image*                    img2,
    IMAGE_EXPORT_DIRECTORY*      export_dir,
    uint32_t*                    orig_thunk_table,
  ...
  while (*orig_thunk_table) {
    ...
    if (*orig_thunk_table & 0x80000000)
      ordinal = (*orig_thunk_table & ~0x80000000) -
                1;  // FIXME - make sure not out of bounds
    else {
      for (unsigned int i = 0; i < export_dir->NumberOfNames; i++) {
        char* export_name = (char*)((uint8_t*)img2->pub.Data + name_table[i]);
...
resolve_imports(EFI_PE_IMAGE*                This,
                char*                        LibraryName,
                EFI_PE_IMAGE*                Library,
                EFI_PE_IMAGE_RESOLVE_FORWARD ResolveForward) {
  EFI_STATUS        Status;
  pe_image*         img        = _CR(This, pe_image, pub);
  ...
  if (nt_header2->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
    if (nt_header2->OptionalHeader64.NumberOfRvaAndSizes <=
            IMAGE_DIRECTORY_ENTRY_EXPORT ||
        nt_header2->OptionalHeader64.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]
                .VirtualAddress == 0 ||
        nt_header2->OptionalHeader64.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]
    ...
    export_dir =
        (IMAGE_EXPORT_DIRECTORY*)((uint8_t*)img2->pub.Data +
                                  nt_header2->OptionalHeader64
                                      .DataDirectory
                                          [IMAGE_DIRECTORY_ENTRY_EXPORT]
                                      .VirtualAddress);
  } else {
    if (nt_header2->OptionalHeader32.NumberOfRvaAndSizes <=
            IMAGE_DIRECTORY_ENTRY_EXPORT ||
        nt_header2->OptionalHeader32.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]
                .VirtualAddress == 0 ||
        nt_header2->OptionalHeader32.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]
                .Size < sizeof(IMAGE_EXPORT_DIRECTORY)) {
      print_string("Exports list not found.\n");
  ...
  for (unsigned int i = 0; i < num_entries; i++) {
    ...
    if (!stricmp(name, LibraryName)) {
      if (nt_header->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
        uint64_t* orig_thunk_table =
            (uint64_t*)((uint8_t*)img->pub.Data + iid[i].Characteristics);
        uint64_t* thunk_table =
            (uint64_t*)((uint8_t*)img->pub.Data + iid[i].FirstThunk);

        Status = resolve_imports2_64(img, img2, export_dir, orig_thunk_table,
                                     thunk_table, ResolveForward);
        if (EFI_ERROR(Status))
          return Status;
      } else {
        uint32_t* orig_thunk_table =
            (uint32_t*)((uint8_t*)img->pub.Data + iid[i].Characteristics);
        uint32_t* thunk_table =
            (uint32_t*)((uint8_t*)img->pub.Data + iid[i].FirstThunk);

        Status = resolve_imports2_32(img, img2, export_dir, orig_thunk_table,
                                     thunk_table, ResolveForward);
        if (EFI_ERROR(Status))
          return Status;
      }
...
static void do_relocations(pe_image* img, IMAGE_NT_HEADERS* nt_header) {
  ...
  if (nt_header->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
    ...
    size = nt_header->OptionalHeader64
               .DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]
               .Size;

    reloc = (IMAGE_BASE_RELOCATION*)((uint8_t*)img->pub.Data +
                                     nt_header->OptionalHeader64
                                         .DataDirectory
                                             [IMAGE_DIRECTORY_ENTRY_BASERELOC]
                                         .VirtualAddress);
    ...
    size = nt_header->OptionalHeader32
               .DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]
               .Size;
  ...
  do {
    ...
    count =
        (reloc->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) / sizeof(uint16_t);
...
static void randomize_security_cookie(pe_image*         img,
                                      IMAGE_NT_HEADERS* nt_header) {
  uint32_t size;
  ...
  if (nt_header->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
    ...
    config =
        (IMAGE_LOAD_CONFIG_DIRECTORY64*)((uint8_t*)img->pub.Data +
                                         nt_header->OptionalHeader64
                                             .DataDirectory
                                                 [IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG]
                                             .VirtualAddress);
    ...
    if (size < offsetof(IMAGE_LOAD_CONFIG_DIRECTORY64, SecurityCookie) +
                   sizeof(uint64_t))
      return;
    ...
    cookie = (uint64_t*)((uint8_t*)img->pub.Data + config->SecurityCookie -
                         (uint8_t*)img->va);
    ...
    *cookie &= 0xffffffffffff;
    ...
    config =
        (IMAGE_LOAD_CONFIG_DIRECTORY32*)((uint8_t*)img->pub.Data +
                                         nt_header->OptionalHeader32
                                             .DataDirectory
                                                 [IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG]
                                             .VirtualAddress);
    ...
    if (size < offsetof(IMAGE_LOAD_CONFIG_DIRECTORY32, SecurityCookie) +
                   sizeof(uint32_t))
      return;
    ...
    cookie  = (uint32_t*)((uint8_t*)img->pub.Data + config->SecurityCookie -
                         (uint8_t*)img->va);
...
static EFI_STATUS EFIAPI move_address(EFI_PE_IMAGE*        This,
...
static EFI_STATUS get_version4(VS_VERSION_INFO* ver,
                               uint32_t         size,
                               uint32_t*        version_ms,
                               uint32_t*        version_ls) {
...
static EFI_STATUS get_version3(pe_image* img,
                               void*     res,
                               uint32_t  ressize,
                               uint32_t  offset,
                               uint32_t* version_ms,
  ...
  ents = (IMAGE_RESOURCE_DIRECTORY_ENTRY*)((uint8_t*)resdir +
  ...
  for (unsigned int i = 0; i < resdir->NumberOfIdEntries; i++) {
    ...
    irde = (IMAGE_RESOURCE_DATA_ENTRY*)((uint8_t*)res +
                                        ents[resdir->NumberOfNamedEntries + i]
                                            .OffsetToData);
    ...
    return get_version4(
        (VS_VERSION_INFO*)((uint8_t*)img->pub.Data + irde->OffsetToData),
        irde->Size, version_ms, version_ls);
  }

  return EFI_NOT_FOUND;
}

static EFI_STATUS get_version2(pe_image* img,
                               void*     res,
                               uint32_t  ressize,
                               uint32_t  offset,
                               uint32_t* version_ms,
  ...
  for (unsigned int i = 0; i < resdir->NumberOfIdEntries; i++) {
    ...
    Status =
        get_version3(img, res, ressize,
                     ents[resdir->NumberOfNamedEntries + i].OffsetToDirectory,
...
static EFI_STATUS EFIAPI get_version(EFI_PE_IMAGE* This,
                                     UINT32*       VersionMS,
                                     UINT32*       VersionLS) {
  EFI_STATUS        Status;
  pe_image*         img        = _CR(This, pe_image, pub);
  ...
  if (nt_header->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
    ...
    resdir =
        (IMAGE_RESOURCE_DIRECTORY*)((uint8_t*)img->pub.Data +
                                    nt_header->OptionalHeader64
                                        .DataDirectory
                                            [IMAGE_DIRECTORY_ENTRY_RESOURCE]
                                        .VirtualAddress);

    if (nt_header->OptionalHeader64
            .DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE]
            .Size <
        sizeof(IMAGE_RESOURCE_DIRECTORY) +
            ((resdir->NumberOfNamedEntries + resdir->NumberOfIdEntries) *
    ...
    dirsize = nt_header->OptionalHeader64
                  .DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE]
                  .Size;
  } else {
    if (nt_header->OptionalHeader32.NumberOfRvaAndSizes <=
    ...
    dirsize = nt_header->OptionalHeader32
                  .DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE]
                  .Size;
  }
  ...
  for (unsigned int i = 0; i < resdir->NumberOfIdEntries; i++) {
    if (ents[resdir->NumberOfNamedEntries + i].Id == RT_VERSION) {
      ...
      if (nt_header->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
        addr =
            (uint8_t*)addr + nt_header->OptionalHeader64
                                 .DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE]
                                 .VirtualAddress;
      else
        addr =
            (uint8_t*)addr + nt_header->OptionalHeader32
                                 .DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE]
                                 .VirtualAddress;
      ...
      Status =
          get_version2(img, resdir, dirsize,
                       ents[resdir->NumberOfNamedEntries + i].OffsetToDirectory,
                       VersionMS, VersionLS);
...
find_export(EFI_PE_IMAGE*                This,
  ...
  if (nt_header->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
    if (nt_header->OptionalHeader64.NumberOfRvaAndSizes <=
            IMAGE_DIRECTORY_ENTRY_EXPORT ||
        nt_header->OptionalHeader64.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]
                .VirtualAddress == 0 ||
        nt_header->OptionalHeader64.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]
                .Size < sizeof(IMAGE_EXPORT_DIRECTORY)) {
      print_string("Exports list not found.\n");
    ...
    export_dir =
        (IMAGE_EXPORT_DIRECTORY*)((uint8_t*)img->pub.Data +
                                  nt_header->OptionalHeader64
                                      .DataDirectory
                                          [IMAGE_DIRECTORY_ENTRY_EXPORT]
                                      .VirtualAddress);
    ...
    if (nt_header->OptionalHeader32.NumberOfRvaAndSizes <=
            IMAGE_DIRECTORY_ENTRY_EXPORT ||
        nt_header->OptionalHeader32.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]
                .VirtualAddress == 0 ||
        nt_header->OptionalHeader32.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]
                .Size < sizeof(IMAGE_EXPORT_DIRECTORY)) {
      print_string("Exports list not found.\n");
  ...
  ordinal_table =
      (uint16_t*)((uint8_t*)img->pub.Data + export_dir->AddressOfNameOrdinals);
  name_table =
      (uint32_t*)((uint8_t*)img->pub.Data + export_dir->AddressOfNames);
  function_table =
      (uint32_t*)((uint8_t*)img->pub.Data + export_dir->AddressOfFunctions);
        ...
        function_table[ordinal] <
            nt_header->OptionalHeader64
                ...
                nt_header->OptionalHeader64
                    ...
                    .Size) {  // forwarded
      char* redir_name =
          (char*)((uint8_t*)img->pub.Data + function_table[ordinal]);
...
static EFI_STATUS EFIAPI get_sections(EFI_PE_IMAGE*          This,
  ...
  *Sections =
      (IMAGE_SECTION_HEADER*)((uint8_t*)&nt_header->OptionalHeader32 +
...
static EFI_STATUS relocate(EFI_PE_IMAGE* This, EFI_VIRTUAL_ADDRESS Address) {
  ...
  img->va =
      (void*)(uintptr_t)(Address - old_va +
                         base);  // because do_relocations works on offsets
...
static EFI_STATUS EFIAPI Load(EFI_FILE_HANDLE File,
  ...
  Status =
      bs->AllocatePages(AllocateAnyPages, EfiLoaderData, img->pages, &addr);
  if (EFI_ERROR(Status)) {
    print_error("AllocatePages", Status);
    bs->FreePages((EFI_PHYSICAL_ADDRESS)(uintptr_t)data, pages);
    bs->FreePool(img);
  ...
  if (nt_header->OptionalHeader32.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
    sections =
        (IMAGE_SECTION_HEADER*)((uint8_t*)&nt_header->OptionalHeader64 +
                                nt_header->FileHeader.SizeOfOptionalHeader);
  ...
  for (unsigned int i = 0; i < nt_header->FileHeader.NumberOfSections; i++) {
    ...
    if (section_size > 0 && sections[i].PointerToRawData != 0)
      memcpy((uint8_t*)img->pub.Data + sections[i].VirtualAddress,
             data + sections[i].PointerToRawData, section_size);

    if (section_size < sections[i].VirtualSize)  // if short, pad with zeroes
      memset(
          (uint8_t*)img->pub.Data + sections[i].VirtualAddress + section_size,
          0, sections[i].VirtualSize - section_size);
  }
