...
EFI_STATUS info_register(EFI_BOOT_SERVICES* bs) {
  ...
  return bs->InstallProtocolInterface(&info_handle, &info_guid,
                                      EFI_NATIVE_INTERFACE, &info_proto);
}
...
static void move_up_console(unsigned int delta) {
  ...
  memcpy(dest, src,
         gop_info.PixelsPerScanLine * (gop_info.VerticalResolution - delta) *
             sizeof(uint32_t));
  dest += gop_info.PixelsPerScanLine * (gop_info.VerticalResolution - delta);

  memset(dest, 0,
         gop_info.PixelsPerScanLine * delta * sizeof(uint32_t));  // black

  memcpy(framebuffer, shadow_fb, framebuffer_size);
}

void draw_text_ft(std::string_view sv,
                  text_pos&        p,
                  uint32_t         bg_colour,
                  uint32_t         fg_colour) {
  FT_Error     error;
  ...
  while (start < sv.size()) {
    ...
    hb_buffer_add_utf8(buf.get(), (char*)sv.data(), sv.size(), start,
                       end - start);
    ...
    if (p.x + width > gop_info.HorizontalResolution) {
      ...
      auto find_break = [&]() {
        while (true) {
          auto space =
              std::string_view(sv.data() + start, e - start).rfind(" ");
      ...
      // if not handled but could fit at least one word on new line, add new
      // line straightaway
    ...
    for (unsigned int i = 0; i < glyph_count; i++) {
      error =
          FT_Load_Glyph(face, glyph_info[i].codepoint, FT_RENDER_MODE_NORMAL);
      if (error) {
      ...
      int rect_left =
          bg_x + face->glyph->bitmap_left + (glyph_pos[i].x_offset / 64);
      int rect_top =
          bg_y - face->glyph->bitmap_top - (glyph_pos[i].y_offset / 64);
      ...
      auto base = (uint32_t*)framebuffer +
                  (rect_top * gop_info.PixelsPerScanLine) + rect_left;
      auto shadow_base = (uint32_t*)(((uint8_t*)base - (uint8_t*)framebuffer) +
                                     (uint8_t*)shadow_fb);
          ...
          for (unsigned int x = skip_x; x < width; x++) {
            if constexpr (std::is_same_v<T, uint32_t>) {
              ...
              if (alpha == 255)
                ...
                uint16_t r = (bg_r * (255 - alpha)) +
                             (((*buf & 0xff0000) >> 16) * alpha);
                uint16_t g =
                    (bg_g * (255 - alpha)) + (((*buf & 0xff00) >> 8) * alpha);
                uint16_t b = (bg_b * (255 - alpha)) + ((*buf & 0xff) * alpha);

                base[x] = shadow_base[x] =
...
EFI_STATUS load_font() {
  ...
  error =
      FT_New_Memory_Face(ft, (const FT_Byte*)font_data, font_size, 0, &face);
  if (error) {
    char s[255], *p;
  ...
  hb_blob = hb_blob_create((const char*)font_data, font_size,
  ...
  do {
    if (h->free) {
      ...
      if (h->size > size) {
        ...
        if (&nh <
            (void*)((uintptr_t)ft_pool + (FT_POOL_PAGES << EFI_PAGE_SHIFT))) {
          nh.free = 1;
...
static void* ft_realloc(FT_Memory memory,
                        long      cur_size,
                        long      new_size,
                        void*     block) {
  void* ret;
...
extern "C" FT_Memory FT_New_Memory() {
  ...
  Status = systable->BootServices->AllocatePages(
      AllocateAnyPages, EfiLoaderData, FT_POOL_PAGES, &addr);
  if (EFI_ERROR(Status))
    return nullptr;
...
extern "C" void __imp_longjmp(jmp_buf, int) {
  abort();
}

#elif defined(_X86_)

extern "C" void _imp__longjmp(jmp_buf, int) {
  abort();
}

#endif

extern "C" int _setjmp(jmp_buf, void*) {
  return 0;
}

extern "C" int _setjmp3(jmp_buf, void*) {
  return 0;
}
...
extern "C" int __mingw_vsprintf(char*, const char*, va_list) {
  abort();
}

extern "C" int __mingw_vsnprintf(char*, size_t, const char*, va_list) {
  abort();
}

extern "C" void* hb_malloc_impl2(size_t size) {
  return ft_alloc(0, size);
}

extern "C" void* hb_calloc_impl2(size_t nmemb, size_t size) {
...
extern "C" void* hb_realloc_impl2(void* ptr, size_t new_size) {
  if (!ptr)
...
extern "C" void hb_free_impl2(void* ptr) {
  ft_free(0, ptr);
}
