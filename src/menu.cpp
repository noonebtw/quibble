...
static const wchar_t timeout_messagew[] =
    L"Time until selected option is chosen: ";
    ...
    if (*s == '[') {
      ...
      if (!sect) {  // allocate new section
        Status = systable->BootServices->AllocatePool(
            EfiLoaderData, offsetof(ini_section, name[0]) + sectnamelen + 1,
            (void**)&sect);
        if (EFI_ERROR(Status)) {
      ...
      Status = systable->BootServices->AllocatePool(
          EfiLoaderData, sizeof(ini_value) + namelen + 1 + valuelen + 1,
          (void**)&item);
      if (EFI_ERROR(Status)) {
        print_error("AllocatePool", Status);
        return Status;
...
static EFI_STATUS draw_box(EFI_SIMPLE_TEXT_OUT_PROTOCOL* con,
                           unsigned int                  x,
                           unsigned int                  y,
                           unsigned int                  w,
                           unsigned int                  h) {
  ...
  Status =
      bs->AllocatePool(EfiLoaderData, (w + 1) * sizeof(wchar_t), (void**)&s);
  if (EFI_ERROR(Status)) {
    print_error("AllocatePool", Status);
    return Status;
  }
...
static EFI_STATUS draw_option(EFI_SIMPLE_TEXT_OUT_PROTOCOL* con,
                              unsigned int                  pos,
                              unsigned int                  width,
                              wchar_t*                      text,
                              bool                          selected) {
  ...
  Status = bs->AllocatePool(EfiLoaderData, (width + 1) * sizeof(wchar_t),
                            (void**)&s);
  if (EFI_ERROR(Status)) {
    print_error("AllocatePool", Status);
    return Status;
...
static EFI_STATUS draw_options(EFI_SIMPLE_TEXT_OUT_PROTOCOL* con,
                               unsigned int                  cols) {
  EFI_STATUS Status;
...
static EFI_STATUS print_spaces(EFI_SIMPLE_TEXT_OUT_PROTOCOL* con,
                               unsigned int                  num) {
  EFI_STATUS Status;
...
static void draw_box_gop(unsigned int x,
                         unsigned int y,
                         unsigned int w,
                         unsigned int h) {
  uint32_t* base;

  memset((uint32_t*)framebuffer + (y * gop_info.PixelsPerScanLine) + x, 0xff,
         w * sizeof(uint32_t));
  memset((uint32_t*)framebuffer + ((y + h) * gop_info.PixelsPerScanLine) + x,
         0xff, w * sizeof(uint32_t));
...
static void draw_rect(unsigned int x,
                      unsigned int y,
                      unsigned int w,
                      unsigned int h,
                      uint32_t     colour) {
...
EFI_STATUS show_menu(EFI_SYSTEM_TABLE* systable, boot_option** ret) {
  ...
  if (gop_console) {
    ...
    memset(framebuffer, 0,
           gop_info.PixelsPerScanLine * gop_info.VerticalResolution *
               4);  // clear screen
  ...
  if (timer > 0) {
    ...
    if (gop_console) {
      ...
      draw_box_gop(font_height, font_height * 3,
                   gop_info.HorizontalResolution - (font_height * 2),
                   gop_info.VerticalResolution - (font_height * 5));
    ...
    do {
      ...
      if (index == 0) {  // timer
        ...
        if (gop_console) {
          ...
          Status = con->SetCursorPosition(
              con,
              (sizeof(timeout_messagew) - sizeof(wchar_t)) / sizeof(wchar_t),
              rows - 1);
          if (EFI_ERROR(Status)) {
            print_error("SetCursorPosition", Status);
            return Status;
          ...
          Status = print_spaces(
              con, cols -
                       (sizeof(timeout_messagew) - sizeof(wchar_t)) /
                           sizeof(wchar_t) -
                       1);
          if (EFI_ERROR(Status)) {
        ...
        if (!timer_cancelled) {
          ...
          if (gop_console) {
            unsigned int y =
                gop_info.VerticalResolution - (font_height * 7 / 4);
  ...
  if (gop_console) {
    memset(framebuffer, 0,
           gop_info.PixelsPerScanLine * gop_info.VerticalResolution *
               4);  // clear screen
    memset(shadow_fb, 0,
           gop_info.PixelsPerScanLine * gop_info.VerticalResolution * 4);
