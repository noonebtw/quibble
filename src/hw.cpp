...
static EFI_STATUS add_ccd(EFI_BOOT_SERVICES*             bs,
                          CONFIGURATION_COMPONENT_DATA*  parent,
                          ...
                          CM_PARTIAL_RESOURCE_LIST*      resource_list,
                          uint32_t                       resource_list_size,
                          void**                         va,
                          LIST_ENTRY*                    mappings,
                          CONFIGURATION_COMPONENT_DATA** pccd) {
  EFI_STATUS                    Status;
  ...
  if (identifier_string) {
    ...
    ccd->ComponentEntry.Identifier =
        (char*)((uint8_t*)ccd + sizeof(CONFIGURATION_COMPONENT_DATA));

    memcpy(ccd->ComponentEntry.Identifier, identifier_string,
           ccd->ComponentEntry.IdentifierLength);
  }

  if (resource_list) {
    ccd->ConfigurationData = (uint8_t*)ccd +
  ...
  Status =
      add_mapping(bs, mappings, *va, ccd, page_count(size), LoaderSystemBlock);
  if (EFI_ERROR(Status)) {
...
static EFI_STATUS add_acpi_config_data(EFI_BOOT_SERVICES*            bs,
                                       CONFIGURATION_COMPONENT_DATA* parent,
                                       void**                        va,
                                       LIST_ENTRY*                   mappings,
                                       uint16_t                      version) {
  ...
  for (unsigned int i = 0; i < systable->NumberOfTableEntries; i++) {
    if (!memcmp(&systable->ConfigurationTable[i].VendorGuid, &acpi2_guid,
                sizeof(EFI_GUID))) {
  ...
  if (!rsdp) {
    for (unsigned int i = 0; i < systable->NumberOfTableEntries; i++) {
      if (!memcmp(&systable->ConfigurationTable[i].VendorGuid, &acpi1_guid,
                  sizeof(EFI_GUID))) {
        rsdp = (RSDP_DESCRIPTOR*)systable->ConfigurationTable[i].VendorTable;
        break;
      }
  ...
  if (rsdp->revision == 0 ||
      (rsdp->revision == 2 && version < _WIN32_WINNT_WINXP))  // ACPI 1.0
  ...
  table_size = offsetof(ACPI_BIOS_DATA, MemoryMap[0]) +
               (map_count * sizeof(BIOS_MEMORY_MAP));

  Status = bs->AllocatePool(EfiLoaderData,
                            sizeof(CM_PARTIAL_RESOURCE_LIST) + table_size,
                            (void**)&prl);
  if (EFI_ERROR(Status)) {
  ...
  Status = add_ccd(bs, parent, AdapterClass, MultiFunctionAdapter,
                   (IDENTIFIER_FLAG)0, 0, 0xffffffff, "ACPI BIOS", prl,
                   sizeof(CM_PARTIAL_RESOURCE_LIST) + table_size, va, mappings,
                   NULL);
...
static EFI_STATUS create_system_key(EFI_BOOT_SERVICES*             bs,
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    Status = bs->OpenProtocol(handles[i], &guid, (void**)&io, image_handle,
  ...
  size = sizeof(CM_PARTIAL_RESOURCE_LIST) +
         (sizeof(CM_INT13_DRIVE_PARAMETER) * disk_count);
  ...
  prl->PartialDescriptors[0].u.DeviceSpecificData.DataSize =
      sizeof(CM_INT13_DRIVE_PARAMETER) * disk_count;
  ...
  Status = add_ccd(bs, NULL, SystemClass, MaximumType, (IDENTIFIER_FLAG)0, 0,
                   0xffffffff, NULL, prl, size, va, mappings, system_key);
  if (EFI_ERROR(Status)) {
...
static EFI_STATUS add_pci_config(EFI_BOOT_SERVICES*            bs,
                                 CONFIGURATION_COMPONENT_DATA* parent,
                                 void**                        va,
                                 LIST_ENTRY*                   mappings) {
  EFI_STATUS        Status;
  ...
  reslist.prl.PartialDescriptors[0].ShareDisposition =
      CmResourceShareUndetermined;
  reslist.prl.PartialDescriptors[0].Flags = 0;
  reslist.prl.PartialDescriptors[0].u.DeviceSpecificData.DataSize =
  ...
  // FIXME - is it possible to get these values from EFI? These constants are
  // taken from handle_1ab101 in seabios
  ...
  Status = add_ccd(bs, parent, AdapterClass, MultiFunctionAdapter,
                   (IDENTIFIER_FLAG)0, 0, 0xffffffff, "PCI", &reslist.prl,
...
EFI_STATUS find_hardware(EFI_BOOT_SERVICES*             bs,
                         CONFIGURATION_COMPONENT_DATA*& config_root,
                         void**                         va,
                         LIST_ENTRY*                    mappings,
                         EFI_HANDLE                     image_handle,
                         uint16_t                       version) {
...
static EFI_STATUS found_block_device(EFI_BOOT_SERVICES*        bs,
                                     EFI_BLOCK_IO*             io,
  ...
  if (part_num == 0) {
    // FIXME - if neither MBR nor GPT, what should the values of Signature and
    // CheckSum be? Does it matter?
    ...
    if (mbr->Signature == MBR_SIGNATURE) {
      ...
      if (mbr->Partition[0].OSIndicator == 0xEE) {  // GPT
        ...
        Status = io->ReadBlocks(io, io->Media->MediaId, PRIMARY_PART_HEADER_LBA,
                                gpt_size, gpt);
        if (EFI_ERROR(Status)) {
          print_error("io->ReadBlocks", Status);
          bs->FreePool(gpt);
        ...
        if (memcmp(&gpt->Header.Signature, EFI_PTAB_HEADER_ID,
                   sizeof(EFI_PTAB_HEADER_ID) - 1)) {
...
static EFI_STATUS add_isa_key(EFI_BOOT_SERVICES*             bs,
                              CONFIGURATION_COMPONENT_DATA*  parent,
                              void**                         va,
                              LIST_ENTRY*                    mappings,
                              CONFIGURATION_COMPONENT_DATA** ret) {
  return add_ccd(bs, parent, AdapterClass, MultiFunctionAdapter,
                 (IDENTIFIER_FLAG)0, 0, 0xffffffff, "ISA", NULL, 0, va,
                 mappings, ret);
}

static EFI_STATUS add_disk_controller(EFI_BOOT_SERVICES*             bs,
                                      CONFIGURATION_COMPONENT_DATA*  parent,
                                      void**                         va,
                                      LIST_ENTRY*                    mappings,
                                      CONFIGURATION_COMPONENT_DATA** ret) {
  return add_ccd(bs, parent, ControllerClass, DiskController,
                 (IDENTIFIER_FLAG)0, 0, 0xffffffff, NULL, NULL, 0, va, mappings,
                 ret);
}

EFI_STATUS find_disks(EFI_BOOT_SERVICES*            bs,
                      LIST_ENTRY*                   disk_sig_list,
                      void**                        va,
                      LIST_ENTRY*                   mappings,
                      CONFIGURATION_COMPONENT_DATA* system_key,
  ...
  Status = bs->AllocatePages(AllocateAnyPages, EfiLoaderData,
                             page_count(disk_list_size), &addr);
  if (EFI_ERROR(Status)) {
    print_error("AllocatePages", Status);
    return Status;
  }
  ...
  while (le != &block_devices) {
    ...
    if (bd->part_num == 0) {
      if (new_disk_format) {
        ...
        arc->Reserved            = 0;
        memcpy(arc->GptSignature, bd->arc.GptSignature,
               sizeof(arc->GptSignature));
        arc->unknown = 0;
  ...
  Status = add_mapping(bs, mappings, *va, (void*)(uintptr_t)addr,
                       page_count(disk_list_size), LoaderSystemBlock);
  if (EFI_ERROR(Status)) {
    print_error("add_mapping", Status);
    return Status;
  }
  ...
  *va = (uint8_t*)*va + (page_count(disk_list_size) * EFI_PAGE_SIZE);
  ...
  while (le != &block_devices) {
    ...
    if (bd->part_num == 0) {
      ...
      Status = add_ccd(
          bs, diskcon, PeripheralClass, DiskPeripheral,
          (IDENTIFIER_FLAG)(IdentifierFlag_Input | IdentifierFlag_Output), 0,
          0xffffffff, identifier, NULL, 0, va, mappings, NULL);
      if (EFI_ERROR(Status)) {
        print_error("add_ccd", Status);
        return Status;
      }
...
static bool device_path_is_child(EFI_DEVICE_PATH_PROTOCOL* parent,
  ...
  do {
    ...
    pbit =
        (EFI_DEVICE_PATH_PROTOCOL*)((uint8_t*)pbit + *(uint16_t*)pbit->Length);
    cbit =
        (EFI_DEVICE_PATH_PROTOCOL*)((uint8_t*)cbit + *(uint16_t*)cbit->Length);
  } while (true);
}

static EFI_DEVICE_PATH_PROTOCOL* duplicate_device_path(
    EFI_DEVICE_PATH_PROTOCOL* device_path) {
  EFI_STATUS                Status;
  unsigned int              len   = 0;
  ...
  Status =
      systable->BootServices->AllocatePool(EfiLoaderData, len, (void**)&dp);
  if (EFI_ERROR(Status)) {
    print_error("AllocatePool", Status);
    return NULL;
  }
...
static unsigned int get_partition_number(
    EFI_DEVICE_PATH_PROTOCOL* device_path) {
  EFI_DEVICE_PATH_PROTOCOL* dpbit = device_path;
  ...
  do {
    ...
    if (dpbit->Type == MEDIA_DEVICE_PATH &&
        dpbit->SubType == MEDIA_HARDDRIVE_DP) {
...
EFI_STATUS look_for_block_devices(EFI_BOOT_SERVICES* bs) {
  ...
  for (unsigned int time = 0; time < 2; time++) {
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      // FIXME - better way of ignoring CD drives? Does Windows give USB keys
      // etc. an ARC name?

      if (time == 0) {
        if (!io->Media->LogicalPartition &&
            !io->Media->RemovableMedia) {  // non-removable disk
        ...
        if (io->Media->LogicalPartition &&
            !io->Media->RemovableMedia) {  // partition on non-removable disk
          ...
          while (le != &block_devices) {
            ...
            if (bd->part_num == 0 &&
                device_path_is_child(bd->device_path, device_path)) {
      ...
      Status = found_block_device(bs, io, disk_num, part_num,
                                  duplicate_device_path(device_path));
...
EFI_STATUS kdnet_init(EFI_BOOT_SERVICES*       bs,
                      EFI_FILE_HANDLE          dir,
  ...
  wchar_t dll[(sizeof(dll_prefix) / sizeof(wchar_t)) - 1 +
              (sizeof(dll_suffix) / sizeof(wchar_t)) - 1 + 4 + 1];

  memcpy(dll, dll_prefix, sizeof(dll_prefix) - sizeof(wchar_t));
  memcpy(dll + (sizeof(dll_prefix) / sizeof(wchar_t)) - 1 + 4, dll_suffix,
         sizeof(dll_suffix));
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    Status = io->Pci.Read(io, EfiPciIoWidthUint32, 0,
    ...
    if (acpi_dp->Header.Type != ACPI_DEVICE_PATH ||
        acpi_dp->Header.SubType != ACPI_DP ||
        (acpi_dp->HID & PNP_EISA_ID_MASK) != PNP_EISA_ID_CONST) {
    ...
    pci_dp = (PCI_DEVICE_PATH*)((uint8_t*)device_path +
                                *(uint16_t*)acpi_dp->Header.Length);

    if (pci_dp->Header.Type != HARDWARE_DEVICE_PATH ||
    ...
    *ptr = hex_digit(pci.Hdr.VendorId >> 12);
    ptr++;
    *ptr = hex_digit((pci.Hdr.VendorId >> 8) & 0xf);
    ptr++;
    *ptr = hex_digit((pci.Hdr.VendorId >> 4) & 0xf);
    ptr++;
    *ptr = hex_digit(pci.Hdr.VendorId & 0xf);
    ptr++;
    ...
    for (unsigned int j = 0; j < MAXIMUM_DEBUG_BARS; j++) {
      ...
      if (EFI_ERROR(Status)) {
        ...
        if (info->space_descriptor != 0x8a) {  // QWORD address space descriptor
          ...
          ddd->BaseAddress[k].TranslatedAddress =
              (uint8_t*)(uintptr_t)info->address_minimum;
