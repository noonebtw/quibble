...
extern "C" void wcsncpy(wchar_t* dest, const wchar_t* src, size_t n) {
  size_t i = 0;
...
extern "C" void wcsncat(wchar_t* dest, const wchar_t* src, size_t n) {
  while (*dest != 0) {
    if (n == 0)
      return;
...
EFI_STATUS utf8_to_utf16(wchar_t*      dest,
  ...
  for (unsigned int i = 0; i < src_len; i++) {
    ...
    if (!(in[i] & 0x80))
      ...
      if (i >= src_len - 2 || (in[i + 1] & 0xc0) != 0x80 ||
          (in[i + 2] & 0xc0) != 0x80) {
        cp     = 0xfffd;
        Status = EFI_INVALID_PARAMETER;
      } else {
      ...
      if (i >= src_len - 3 || (in[i + 1] & 0xc0) != 0x80 ||
